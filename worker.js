async function ensureSchema(db) {
  ...
  for (const col of ["bigBlind REAL DEFAULT 0", "place REAL DEFAULT 0", "bounties REAL DEFAULT 0", "players REAL DEFAULT 0"]) {
...
async function handleGet(env) {
  const { results } = await env.DB.prepare(
    "SELECT id, date, gameType, stakes, location, startTime, endTime, buyIn, rebuy, cashOut, expenses, notes, bigBlind, place, bounties, players FROM sessions ORDER BY date, startTime"
  ).all();
...
async function handlePost(request, env) {
  ...
  for (const s of sessions) {
    stmts.push(
      env.DB.prepare(
        `INSERT INTO sessions (id, date, gameType, stakes, location, startTime, endTime, buyIn, rebuy, cashOut, expenses, notes, bigBlind, place, bounties, players)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
      ).bind(
